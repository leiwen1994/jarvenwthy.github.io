
public class FaultyFunctions {	

	/** 
	 * selection sort an array
	 *  
	 * @param array
	 * @return
	 */
	static void selectionSort (int[] array) {
		int i = 0;
		while (i <= array.length-1) {	
		    int idxMin = i;
			int j = i+1;
			do{
				if(array[idxMin] < array[j]) {
					idxMin = j;
				}
				j++;
			} while (j < array.length) ;

			if(i != idxMin) {
				swap(array, i, idxMin);
			}
			
			i++;
		}
			
		printArray(array);
	}	
	
	/** 
	 * Set an element i of a new array to the partial sum from the first element 
	 * to the element at i of the given array
	 *   
	 * @param array
	 */
	static int[] partialSum (int[] array) {
		int[] pSumArray = new int[array.length];
		
		for(int idx = 1; idx < array.length ; idx++) {
			pSumArray[idx] = pSumArray[idx-1] + array[idx]; 
		} 
		
		//printArray(pSumArray);
		return pSumArray;
	}	
	

	/**
	 * swap two array elements at position i and j
	 * 
	 * @param array
	 * @param i
	 * @param j
	 */
	static void swap (int[] array, int i, int j) {
		int tmp = array[i];
		array[i] = array[j];
		array[j] = tmp;
	}
	
	/**
	 * Use this to explain System.out.println method
	 * 
	 * @param array
	 */
	static void printArray (int[] array) {
		System.out.println("[");
		
		for (int i = 0; i < array.length; i++) {
			if (i > 0) {
				System.out.println("[" + array[i] + "]");
				
			}
			
		
		}
	}
}


